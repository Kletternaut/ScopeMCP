...
class ScopeController:
    ...
    def get_device(self):
        ...
        if self._dev:
            try:
                # Teste ob Device noch erreichbar
                self._dev.idVendor
                return self._dev
            except Exception:
    ...
    def send_cmd(self, dev, cmd, clear_buffer=True):
        """Sende Befehl im Owon-Binär-Format."""
    ...
    def read_resp(self, dev, size=USB_READ_SIZE_LARGE, timeout=800):
        """Liest Antwort vom Gerät mit Retry-Logik."""
...
async def get_live_metadata() -> dict:
    ...
    # Erzwinge Refresh bei explizitem Aufruf
...
async def set_vertical_scale(channel: int, scale: str) -> str:
    ...
    if channel not in (1, 2): return f"Invalid channel: {channel}"
    dev = await asyncio.to_thread(scope.get_device)
    if not dev: return "No Device"
    scope._meta_cache = None
