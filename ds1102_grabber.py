...
def main():
    ...
    try:
        ...
        # Startwerte - werden in der ersten Loop-Iteration durch Geraetedaten ersetzt
        v_scale_str  = "1.00V"
        v_scale      = 1.0
        offset       = 0.0
        probe_factor = 1.0
        ...
        while plt.fignum_exists(fig.number):
            # 2. Header abfragen (Skalierung + OFFSET)
            ...
            if wave_raw:
                # Verifizierte Formel: (raw - OFFSET) / 250.0 * scale_v * probe_factor
                volt_data = decode_and_convert(wave_raw, offset, v_scale, probe_factor)
